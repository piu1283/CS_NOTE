...
public class PhaserExample {
    ...
    public static void main(String[] args) throws Exception {
        ...
        synchronized (PhaserExample.class) {
            PhaserExample.class.wait();
        }
    }
    ...
    static Phaser ph = new Phaser() {
        ...
        protected boolean onAdvance(int phase, int registeredParties) {
            System.out.printf("第(%d)局，剩余[%d]人", phase, registeredParties);
            ...
            while (!ph.isTerminated() && phase < 100) {
                ...
                if (state == 0) {
                    if (Decide.goon()) {
                        ...
                        if (h < 0)
                            System.out.printf("No%d.[%s]继续，但已胜利。。。", phase, name);
                        else
                            System.out.printf("No%d.[%s]继续at(%d)。。。", phase, name, h);
                    } else {
                        state = -1;
                        h = ph.arriveAndDeregister();
                        System.out.printf("No%d.[%s]退出at(%d)。。。", phase, name, h);
                    }
                } else {
                    if (Decide.revive()) {
                        state = 0;
                        h = ph.register();
                        if (h < 0)
                            System.out.printf("No%d.[%s]复活，但已失败。。。", phase, name);
                        else
                            System.out.printf("No%d.[%s]复活at(%d)。。。", phase, name, h);
                    } else {
                        System.out.printf("No%d.[%s]没有复活。。。", phase, name);
                    }
                }
                phase++;
            }
